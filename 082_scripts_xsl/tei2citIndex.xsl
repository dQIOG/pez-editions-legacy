...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="vide2ref" as="item()*">
            <xsl:apply-templates select="$IDsAdded" mode="vide2ref">
                <xsl:with-param name="IDsAdded" select="$IDsAdded" tunnel="yes"/>
            </xsl:apply-templates>
        </xsl:variable>
        <xsl:sequence select="$vide2ref"/>
    </xsl:template>
    ...
    <xsl:template match="body" mode="indexes">
        <xsl:copy>
            ...
            <xsl:for-each-group select="*" group-starting-with="p[@rend = 'Überschrift Ebene 2']">
                <xsl:if test="current-group()[1]/self::p[@rend  = 'Überschrift Ebene 2'][contains(.,'Redensarten, Zitate, Metaphern')]">
                    <list type="index" n="1">
...
-->    
    ...
    <xsl:template match="list[@type = 'indexEntry'][@n = '2']/item[starts-with(term,'Vide')]" mode="vide2ref">
        <xsl:param name="IDsAdded" as="item()" tunnel="yes"/>
        <xsl:variable name="target" select="$IDsAdded//item[starts-with(term, replace(current()/term,'^Vide\s+',''))]/@xml:id"/>
        <xsl:copy>
            <ref target="{if ($target != '') then concat('#',$target) else 'VERWEIS??'}">
                <xsl:value-of select="."/>
            </ref>
        </xsl:copy>
    </xsl:template>
